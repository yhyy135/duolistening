...
test("posts the prompt to the OpenAI-compatible chat endpoint", async () => {
...
test("gives up after the attempt budget, keeping the reason", async () => {
...
test("does not retry a bad key — that will never fix itself", async () => {
...
test("treats a response with no content as a bad response", async () => {
...
test("completeJson repairs once when the reply is not JSON at all", async () => {
...
test("completeJson gives up after one repair rather than looping", async () => {
...
test("completeStream yields each delta, reassembling a line split across a chunk boundary", async () => {
...
test("completeStream skips a line it cannot parse instead of dying", async () => {
  const fetch = stubFetch(
...
test("completeStream rejects on a bad status before yielding anything", async () => {
  const fetch = stubFetch(fails(401));
...
test("completeStream does not retry — a partial answer already on screen should not be redone", async () => {
  const fetch = stubFetch(fails(503));
  await assert.rejects(collect(model(fetch).completeStream("hi")));
  assert.equal(fetch.calls.length, 1);
});

test("listModels lists model ids from the OpenAI-compatible /models shape, sorted", async () => {
  const fetch = stubFetch(
...
test("listModels sends the key as a bearer token, same as every other call", async () => {
...
test("listModels rejects with the same reasons a chat call would", async () => {
  const fetch = stubFetch(fails(401));
...
test("listModels rejects rather than calling out with no base URL", async () => {
...
test("listModels rejects a response with no data array", async () => {
