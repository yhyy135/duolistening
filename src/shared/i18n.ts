// Every user-facing string, in every language the settings offer. The UI language
// follows the user's Native Language: someone translating into Spanish is reading
// Spanish, so the buttons around the translation may as well be Spanish too.
//
// Shared rather than web-only because one of these strings is not UI at all — the
// ask-AI prompt is written in the native language and sent from the server, and
// having two copies of that template is how they drift apart.

import { LANGUAGE_NAMES, type LanguageCode } from "./model.ts";

type Vars = Record<string, string | number>;

const en = {
  "ask.prompt": "Help me understand this sentence: {text}",

  "theme.auto": "Auto",
  "theme.light": "Light",
  "theme.dark": "Dark",
  "nav.settings": "Settings",
  "nav.library": "Library",
  "nav.search": "Search or import",

  "common.loading": "Loading…",
  "common.back": "Back",
  "common.save": "Save",
  "common.saving": "Saving…",
  "common.saved": "Saved",
  "common.delete": "Delete",
  "common.confirm": "Confirm",
  "common.cancel": "Cancel",
  "common.close": "Close",
  "common.on": "On",
  "common.off": "Off",

  "phase.queued": "Queued",
  "phase.fetching": "Downloading",
  "phase.transcribing": "Transcribing",
  "phase.annotating": "Translating",
  "phase.ready": "Ready",
  "phase.failed": "Failed",
  "phase.untranscribed": "Audio only",

  "onboard.title": "Start in three steps",
  "onboard.proxy": "Fill in the byte proxy in Settings. Every download goes through it.",
  "onboard.transcribe":
    "Optional: add a Transcription Model, and each import arrives with a transcript. Without one you can already listen.",
  "onboard.pick": "Pick a show below, then choose an episode to import.",
  "onboard.done": "done",

  "library.needsSettings": "Set up the byte proxy in Settings first.",
  "library.continue": "Continue listening",
  "library.left": "{time} left",
  "library.finished": "Finished",
  "library.exportBackup": "Export",
  "library.importBackup": "Restore",
  "library.imported": "{added} added, {skipped} already here.",
  "library.retry": "Retry",
  "library.resumeImport": "Resume",
  "library.working": "Working…",
  "library.autoLanguage": "auto",
  "library.pickEpisode": "Pick an episode to import",
  "library.favorite": "Favorite",
  "library.unfavorite": "Unfavorite",
  "library.columnTitle": "Episode title",
  "library.columnDuration": "Duration",
  "library.fullTitle": "Show the full title",
  "library.feedFailed": "Could not load the episode list: {reason}",

  "discover.title": "Recommended podcasts",
  "discover.subtitle":
    "Shows in the language you are studying, from Apple Podcasts. Refreshed once a day.",
  "discover.loading": "Finding podcasts…",
  "discover.failed": "Could not reach the podcast directory.",
  "discover.empty": "Nothing came back for that language.",
  "discover.favorites": "Favorites",
  "discover.favoritesSubtitle": "My favorite podcasts",
  "discover.noFavorites": "No favorite shows yet",
  "discover.searchPlaceholder": "Search podcasts, or paste an RSS link",
  "discover.results": "Results for “{term}”",
  "discover.clearSearch": "Clear search",
  "discover.languageLearning": "Language learning",

  "player.translating": "Translating…",
  "player.missing": "That episode is no longer in this Library.",
  "player.noTranscript": "No transcript yet.",
  "player.needsTranscription":
    "No transcript yet. Set up a Transcription Model in Settings and try again.",
  "player.retryTranscription": "Retry transcription",
  "player.play": "Play",
  "player.playing": "Playing",
  "player.paused": "Paused",
  "player.position": "Position",
  "player.pause": "Pause",
  "player.previousLine": "Previous line",
  "player.nextLine": "Next line",
  "player.lineOf": "Line {n} of {total}",
  "player.jumpToCurrent": "Jump to current line",
  "player.replay": "Replay this line",
  "player.replayShort": "Replay",
  "player.repeat": "Repeat",
  "player.repeatTitle": "Repeat the current line",
  "player.speed": "Playback speed",
  "player.subtitles": "Subtitles",
  "player.showBoth": "Both",
  "player.showText": "Original",
  "player.showNone": "Hidden",
  "player.revealHint": "Lines are hidden. Choose one to replay it and see it.",
  "player.askTitle": "Ask about this line",
  "player.asking": "Asking…",
  "player.needsTextModel": "Set up a Text Model in Settings to ask about a line.",
  "player.keyPlay": "play",
  "player.keyLine": "line",
  "player.keyReplay": "replay",
  "player.keyLoop": "loop",
  "player.keySubtitles": "subtitles",

  "settings.appearance": "Appearance",
  "settings.appearanceHint": "Only on this device, and it applies straight away.",
  "settings.look": "Style",
  "settings.colorMode": "Light or dark",
  "look.standard": "Standard",
  "look.standardHint": "Like a music app's lyric page",
  "look.station": "Station",
  "look.stationHint": "Like a station's departure board",

  "settings.languages": "Languages",
  "settings.proxy": "Byte proxy",
  "settings.proxyKey": "Proxy key",
  "settings.proxyHint": "The Worker that fetches audio for you. Imports need it.",

  "settings.native": "Native Language",
  "settings.studying": "Studying (optional)",
  "settings.autoDetect": "Auto-detect",
  "settings.textModel": "Text Model — translation and the ask-AI popup",
  "settings.transcriptionModel": "Transcription Model — speech to text",
  "settings.provider": "Provider",
  "settings.providerCustom": "Custom",
  "settings.cloudflareHint":
    "Cloudflare Workers AI speaks the OpenAI shape too: replace ACCOUNT_ID in the base URL with yours, and use a model like {model}.",
  "settings.groqHint": "Groq runs Whisper for free — try model whisper-large-v3-turbo.",
  "settings.getApiKey":
    "Create an API key at {host} and paste it below, then click “{action}” to pick one.",
  "settings.getApiKeyLink": "Create an API key →",
  "settings.ollamaHint": "Ollama runs on your own machine — no API key needed.",
  "settings.baseUrl": "Base URL",
  "settings.apiKey": "API key",
  "settings.model": "Model",
  "settings.show": "Show",
  "settings.hide": "Hide",
  "settings.fetchModels": "Fetch models",
  "settings.fetching": "Fetching…",
  "settings.test": "Test connection",
  "settings.testing": "Testing…",
  "settings.answered": "✓ Answered.",
  "settings.modelsFound": "{count} models found.",
  "settings.discard": "Discard unsaved changes?",
  "settings.exportSettings": "Export",
  "settings.importSettings": "Import",
  "settings.exportTitle": "Export Configuration",
  "settings.exportWarning":
    "Below is your configuration string. Copy it and paste it on another device to import the same setup quickly.",
  "settings.copy": "Copy",
  "settings.copied": "Copied",
  "settings.copyFailed": "Could not copy — select the text and copy it yourself.",
  "settings.importTitle": "Quick Import",
  "settings.cipherPlaceholder": "Enter the configuration string",
  "settings.importApply": "Confirm Import",
  "settings.importFailed": "Import failed: could not parse the string.",
  "settings.imported": "Filled in — press Save to keep them.",
};

/** Every other language answers the same keys English does. */
export type StringKey = keyof typeof en;

const STRINGS: Record<LanguageCode, Record<StringKey, string>> = {
  en,

  "zh-CN": {
    "ask.prompt": "帮我理解这个句子：{text}",

    "theme.auto": "自动",
    "theme.light": "浅色",
    "theme.dark": "深色",
    "nav.settings": "设置",
    "nav.library": "书架",
    "nav.search": "搜索或导入",

    "common.loading": "加载中…",
    "common.back": "返回",
    "common.save": "保存",
    "common.saving": "保存中…",
    "common.saved": "已保存",
    "common.delete": "删除",
    "common.confirm": "确定",
    "common.cancel": "取消",
    "common.close": "关闭",
    "common.on": "开",
    "common.off": "关",

    "phase.queued": "排队中",
    "phase.fetching": "下载中",
    "phase.transcribing": "转写中",
    "phase.annotating": "翻译中",
    "phase.ready": "就绪",
    "phase.failed": "失败",
    "phase.untranscribed": "仅音频",

    "onboard.title": "三步开始",
    "onboard.proxy": "在设置里填好字节代理，所有下载都要经过它。",
    "onboard.transcribe": "可选：填好转写模型，导入时会自动生成字幕；不填也能先听。",
    "onboard.pick": "在下面挑一个节目，再选一集导入。",
    "onboard.done": "已完成",

    "library.needsSettings": "请先在设置里配置字节代理。",
    "library.continue": "继续收听",
    "library.left": "剩余 {time}",
    "library.finished": "已听完",
    "library.exportBackup": "导出",
    "library.importBackup": "导入备份",
    "library.imported": "新增 {added} 集，{skipped} 集已存在。",
    "library.retry": "重试",
    "library.resumeImport": "继续",
    "library.working": "处理中…",
    "library.autoLanguage": "自动",
    "library.pickEpisode": "选择要导入的单集",
    "library.favorite": "收藏",
    "library.unfavorite": "取消收藏",
    "library.columnTitle": "节目标题",
    "library.columnDuration": "时长",
    "library.fullTitle": "查看完整标题",
    "library.feedFailed": "获取节目列表失败：{reason}",

    "discover.title": "推荐播客",
    "discover.subtitle": "以你正在学的语言播出的节目，来自 Apple Podcasts。每天更新一次。",
    "discover.loading": "正在查找播客…",
    "discover.failed": "无法连接播客目录。",
    "discover.empty": "这个语言没有找到节目。",
    "discover.favorites": "收藏",
    "discover.favoritesSubtitle": "我收藏的播客节目",
    "discover.noFavorites": "暂无收藏的节目",
    "discover.searchPlaceholder": "搜索播客，或粘贴 RSS 链接",
    "discover.results": "“{term}”的搜索结果",
    "discover.clearSearch": "清除搜索",
    "discover.languageLearning": "语言学习",

    "player.translating": "翻译中…",
    "player.missing": "这一集已不在本地库里。",
    "player.noTranscript": "暂无字幕。",
    "player.needsTranscription": "暂无字幕，请设置转写模型后重试",
    "player.retryTranscription": "重试转写",
    "player.play": "播放",
    "player.playing": "播放中",
    "player.paused": "已暂停",
    "player.position": "播放进度",
    "player.pause": "暂停",
    "player.previousLine": "上一句",
    "player.nextLine": "下一句",
    "player.lineOf": "第 {n} / {total} 句",
    "player.jumpToCurrent": "跳到当前句",
    "player.replay": "重播本句",
    "player.replayShort": "重播",
    "player.repeat": "循环",
    "player.repeatTitle": "循环播放当前句",
    "player.speed": "播放速度",
    "player.subtitles": "字幕",
    "player.showBoth": "双语",
    "player.showText": "原文",
    "player.showNone": "盲听",
    "player.revealHint": "字幕已隐藏。选一句即可重播并查看。",
    "player.askTitle": "询问这一句",
    "player.asking": "提问中…",
    "player.needsTextModel": "请先在设置里配置文本模型，才能就某一句提问。",
    "player.keyPlay": "播放",
    "player.keyLine": "切换句",
    "player.keyReplay": "重播",
    "player.keyLoop": "循环",
    "player.keySubtitles": "字幕",

    "settings.appearance": "外观",
    "settings.appearanceHint": "只影响这台设备，选择后立即生效。",
    "settings.look": "风格",
    "settings.colorMode": "深浅色",
    "look.standard": "标准",
    "look.standardHint": "像音乐 App 的歌词页",
    "look.station": "车站",
    "look.stationHint": "像车站的到站屏",

    "settings.languages": "语言",
    "settings.proxy": "字节代理",
    "settings.proxyKey": "代理密钥",
    "settings.proxyHint": "替你抓取音频的 Worker，导入时必须用到。",

    "settings.native": "母语",
    "settings.studying": "学习语言（选填）",
    "settings.autoDetect": "自动识别",
    "settings.textModel": "文本模型 — 翻译与 AI 问答",
    "settings.transcriptionModel": "转写模型 — 语音转文字",
    "settings.provider": "服务商",
    "settings.providerCustom": "自定义",
    "settings.cloudflareHint":
      "Cloudflare Workers AI 也兼容 OpenAI 格式：把 base URL 里的 ACCOUNT_ID 换成你自己的账号 ID，模型名可以用 {model} 之类。",
    "settings.groqHint": "Groq 免费提供 Whisper，推荐使用 whisper-large-v3-turbo 模型。",
    "settings.getApiKey":
      "在 {host} 创建 API Key 并填入下方，然后点击「{action}」选择模型即可。",
    "settings.getApiKeyLink": "前往创建 API Key →",
    "settings.ollamaHint": "Ollama 运行在你自己的电脑上，不需要 API Key。",
    "settings.baseUrl": "Base URL",
    "settings.apiKey": "API 密钥",
    "settings.model": "模型",
    "settings.show": "显示",
    "settings.hide": "隐藏",
    "settings.fetchModels": "获取模型列表",
    "settings.fetching": "获取中…",
    "settings.test": "测试连接",
    "settings.testing": "测试中…",
    "settings.answered": "✓ 已响应。",
    "settings.modelsFound": "找到 {count} 个模型。",
    "settings.discard": "放弃未保存的修改？",
    "settings.exportSettings": "导出",
    "settings.importSettings": "导入",
    "settings.exportTitle": "导出配置",
    "settings.exportWarning": "下面是配置密文，可复制到其他终端快速导入相同配置",
    "settings.copy": "复制",
    "settings.copied": "已复制",
    "settings.copyFailed": "复制失败，请自己选中文本复制。",
    "settings.importTitle": "快速导入",
    "settings.cipherPlaceholder": "请输入配置密文",
    "settings.importApply": "确认导入",
    "settings.importFailed": "导入失败，密文解析错误",
    "settings.imported": "已填入，按保存才会生效。",
  },

  "zh-TW": {
    "ask.prompt": "幫我理解這個句子：{text}",

    "theme.auto": "自動",
    "theme.light": "淺色",
    "theme.dark": "深色",
    "nav.settings": "設定",
    "nav.library": "書架",
    "nav.search": "搜尋或匯入",

    "common.loading": "載入中…",
    "common.back": "返回",
    "common.save": "儲存",
    "common.saving": "儲存中…",
    "common.saved": "已儲存",
    "common.delete": "刪除",
    "common.confirm": "確定",
    "common.cancel": "取消",
    "common.close": "關閉",
    "common.on": "開",
    "common.off": "關",

    "phase.queued": "排隊中",
    "phase.fetching": "下載中",
    "phase.transcribing": "轉寫中",
    "phase.annotating": "翻譯中",
    "phase.ready": "就緒",
    "phase.failed": "失敗",
    "phase.untranscribed": "僅音訊",

    "onboard.title": "三步開始",
    "onboard.proxy": "在設定裡填好位元組代理，所有下載都要經過它。",
    "onboard.transcribe": "選填：填好轉寫模型，匯入時會自動產生字幕；不填也能先聽。",
    "onboard.pick": "在下面挑一個節目，再選一集匯入。",
    "onboard.done": "已完成",

    "library.needsSettings": "請先在設定裡配置位元組代理。",
    "library.continue": "繼續收聽",
    "library.left": "剩餘 {time}",
    "library.finished": "已聽完",
    "library.exportBackup": "匯出",
    "library.importBackup": "匯入備份",
    "library.imported": "新增 {added} 集，{skipped} 集已存在。",
    "library.retry": "重試",
    "library.resumeImport": "繼續",
    "library.working": "處理中…",
    "library.autoLanguage": "自動",
    "library.pickEpisode": "選擇要匯入的單集",
    "library.favorite": "收藏",
    "library.unfavorite": "取消收藏",
    "library.columnTitle": "節目標題",
    "library.columnDuration": "時長",
    "library.fullTitle": "檢視完整標題",
    "library.feedFailed": "取得節目列表失敗：{reason}",

    "discover.title": "推薦的 Podcast",
    "discover.subtitle": "以你正在學的語言播出的節目，來自 Apple Podcasts。每天更新一次。",
    "discover.loading": "正在尋找 Podcast…",
    "discover.failed": "無法連上 Podcast 目錄。",
    "discover.empty": "這個語言沒有找到節目。",
    "discover.favorites": "收藏",
    "discover.favoritesSubtitle": "我收藏的 Podcast 節目",
    "discover.noFavorites": "暫無收藏的節目",
    "discover.searchPlaceholder": "搜尋 Podcast，或貼上 RSS 連結",
    "discover.results": "「{term}」的搜尋結果",
    "discover.clearSearch": "清除搜尋",
    "discover.languageLearning": "語言學習",

    "player.translating": "翻譯中…",
    "player.missing": "這一集已不在本機庫裡。",
    "player.noTranscript": "尚無字幕。",
    "player.needsTranscription": "暫無字幕，請設定轉寫模型後重試",
    "player.retryTranscription": "重試轉寫",
    "player.play": "播放",
    "player.playing": "播放中",
    "player.paused": "已暫停",
    "player.position": "播放進度",
    "player.pause": "暫停",
    "player.previousLine": "上一句",
    "player.nextLine": "下一句",
    "player.lineOf": "第 {n} / {total} 句",
    "player.jumpToCurrent": "跳到目前這句",
    "player.replay": "重播本句",
    "player.replayShort": "重播",
    "player.repeat": "循環",
    "player.repeatTitle": "循環播放目前這句",
    "player.speed": "播放速度",
    "player.subtitles": "字幕",
    "player.showBoth": "雙語",
    "player.showText": "原文",
    "player.showNone": "盲聽",
    "player.revealHint": "字幕已隱藏。選一句即可重播並查看。",
    "player.askTitle": "詢問這一句",
    "player.asking": "提問中…",
    "player.needsTextModel": "請先在設定裡配置文字模型，才能就某一句提問。",
    "player.keyPlay": "播放",
    "player.keyLine": "換句",
    "player.keyReplay": "重播",
    "player.keyLoop": "循環",
    "player.keySubtitles": "字幕",

    "settings.appearance": "外觀",
    "settings.appearanceHint": "只影響這台裝置，選擇後立即生效。",
    "settings.look": "風格",
    "settings.colorMode": "深淺色",
    "look.standard": "標準",
    "look.standardHint": "像音樂 App 的歌詞頁",
    "look.station": "車站",
    "look.stationHint": "像車站的到站看板",

    "settings.languages": "語言",
    "settings.proxy": "位元組代理",
    "settings.proxyKey": "代理金鑰",
    "settings.proxyHint": "替你抓取音訊的 Worker，匯入時必須用到。",

    "settings.native": "母語",
    "settings.studying": "學習語言（選填）",
    "settings.autoDetect": "自動辨識",
    "settings.textModel": "文字模型 — 翻譯與 AI 問答",
    "settings.transcriptionModel": "轉寫模型 — 語音轉文字",
    "settings.provider": "服務商",
    "settings.providerCustom": "自訂",
    "settings.cloudflareHint":
      "Cloudflare Workers AI 也相容 OpenAI 格式：把 base URL 裡的 ACCOUNT_ID 換成你自己的帳號 ID，模型名可以用 {model} 之類。",
    "settings.groqHint": "Groq 免費提供 Whisper，推薦使用 whisper-large-v3-turbo 模型。",
    "settings.getApiKey":
      "在 {host} 建立 API Key 並貼到下方，然後點擊「{action}」選擇模型即可。",
    "settings.getApiKeyLink": "前往建立 API Key →",
    "settings.ollamaHint": "Ollama 執行在你自己的電腦上，不需要 API Key。",
    "settings.baseUrl": "Base URL",
    "settings.apiKey": "API 金鑰",
    "settings.model": "模型",
    "settings.show": "顯示",
    "settings.hide": "隱藏",
    "settings.fetchModels": "取得模型清單",
    "settings.fetching": "取得中…",
    "settings.test": "測試連線",
    "settings.testing": "測試中…",
    "settings.answered": "✓ 已回應。",
    "settings.modelsFound": "找到 {count} 個模型。",
    "settings.discard": "放棄未儲存的變更？",
    "settings.exportSettings": "匯出",
    "settings.importSettings": "匯入",
    "settings.exportTitle": "匯出配置",
    "settings.exportWarning": "下面是配置密文，可複製到其他裝置快速匯入相同配置",
    "settings.copy": "複製",
    "settings.copied": "已複製",
    "settings.copyFailed": "複製失敗，請自己選取文字複製。",
    "settings.importTitle": "快速匯入",
    "settings.cipherPlaceholder": "請輸入配置密文",
    "settings.importApply": "確認匯入",
    "settings.importFailed": "匯入失敗，密文解析錯誤",
    "settings.imported": "已填入，按儲存才會生效。",
  },

  ja: {
    "ask.prompt": "この文を理解するのを手伝ってください：{text}",

    "theme.auto": "自動",
    "theme.light": "ライト",
    "theme.dark": "ダーク",
    "nav.settings": "設定",
    "nav.library": "ライブラリ",
    "nav.search": "検索・インポート",

    "common.loading": "読み込み中…",
    "common.back": "戻る",
    "common.save": "保存",
    "common.saving": "保存中…",
    "common.saved": "保存しました",
    "common.delete": "削除",
    "common.confirm": "確定",
    "common.cancel": "キャンセル",
    "common.close": "閉じる",
    "common.on": "オン",
    "common.off": "オフ",

    "phase.queued": "待機中",
    "phase.fetching": "ダウンロード中",
    "phase.transcribing": "文字起こし中",
    "phase.annotating": "翻訳中",
    "phase.ready": "準備完了",
    "phase.failed": "失敗",
    "phase.untranscribed": "音声のみ",

    "onboard.title": "3 ステップで始める",
    "onboard.proxy": "設定でバイトプロキシを登録します。ダウンロードはすべてここを通ります。",
    "onboard.transcribe":
      "任意：文字起こしモデルを登録すると、インポート時に字幕が作られます。なくても先に聴けます。",
    "onboard.pick": "下で番組を選び、インポートするエピソードを選びます。",
    "onboard.done": "完了",

    "library.needsSettings": "先に設定でバイトプロキシを登録してください。",
    "library.continue": "続きを聴く",
    "library.left": "残り {time}",
    "library.finished": "聴き終わり",
    "library.exportBackup": "エクスポート",
    "library.importBackup": "バックアップを読み込む",
    "library.imported": "{added} 件を追加、{skipped} 件は既にあります。",
    "library.retry": "再試行",
    "library.resumeImport": "再開",
    "library.working": "処理中…",
    "library.autoLanguage": "自動",
    "library.pickEpisode": "インポートするエピソードを選んでください",
    "library.favorite": "お気に入り",
    "library.unfavorite": "お気に入り解除",
    "library.columnTitle": "エピソード名",
    "library.columnDuration": "長さ",
    "library.fullTitle": "タイトル全体を表示",
    "library.feedFailed": "エピソード一覧を取得できませんでした：{reason}",

    "discover.title": "おすすめのポッドキャスト",
    "discover.subtitle":
      "学習中の言語の番組を Apple Podcasts から集めています。1 日 1 回更新します。",
    "discover.loading": "ポッドキャストを探しています…",
    "discover.failed": "ポッドキャストのディレクトリに接続できませんでした。",
    "discover.empty": "その言語では何も見つかりませんでした。",
    "discover.favorites": "お気に入り",
    "discover.favoritesSubtitle": "お気に入りのポッドキャスト番組",
    "discover.noFavorites": "お気に入りの番組はまだありません",
    "discover.searchPlaceholder": "ポッドキャストを検索、または RSS のリンクを貼り付け",
    "discover.results": "「{term}」の検索結果",
    "discover.clearSearch": "検索をクリア",
    "discover.languageLearning": "語学学習",

    "player.translating": "翻訳中…",
    "player.missing": "そのエピソードはこのライブラリにありません。",
    "player.noTranscript": "字幕はまだありません。",
    "player.needsTranscription":
      "文字起こしがありません。設定で文字起こしモデルを登録してから、もう一度お試しください。",
    "player.retryTranscription": "文字起こしをやり直す",
    "player.play": "再生",
    "player.playing": "再生中",
    "player.paused": "一時停止中",
    "player.position": "再生位置",
    "player.pause": "一時停止",
    "player.previousLine": "前の行",
    "player.nextLine": "次の行",
    "player.lineOf": "{n} / {total} 行",
    "player.jumpToCurrent": "再生中の行へ",
    "player.replay": "この行を頭から再生",
    "player.replayShort": "頭出し",
    "player.repeat": "リピート",
    "player.repeatTitle": "この行をリピート",
    "player.speed": "再生速度",
    "player.subtitles": "字幕",
    "player.showBoth": "対訳",
    "player.showText": "原文",
    "player.showNone": "非表示",
    "player.revealHint": "字幕を隠しています。行を選ぶと、その行をもう一度再生して表示します。",
    "player.askTitle": "この行について質問",
    "player.asking": "質問中…",
    "player.needsTextModel": "行について質問するには、設定でテキストモデルを登録してください。",
    "player.keyPlay": "再生",
    "player.keyLine": "行移動",
    "player.keyReplay": "頭出し",
    "player.keyLoop": "リピート",
    "player.keySubtitles": "字幕",

    "settings.appearance": "外観",
    "settings.appearanceHint": "この端末だけに、すぐに反映されます。",
    "settings.look": "スタイル",
    "settings.colorMode": "ライト・ダーク",
    "look.standard": "スタンダード",
    "look.standardHint": "音楽アプリの歌詞画面のように",
    "look.station": "駅",
    "look.stationHint": "駅の発車案内のように",

    "settings.languages": "言語",
    "settings.proxy": "バイトプロキシ",
    "settings.proxyKey": "プロキシキー",
    "settings.proxyHint": "音声を取得する Worker。インポートに必要です。",

    "settings.native": "母語",
    "settings.studying": "学習中の言語（任意）",
    "settings.autoDetect": "自動判定",
    "settings.textModel": "テキストモデル — 翻訳と AI 質問",
    "settings.transcriptionModel": "文字起こしモデル — 音声認識",
    "settings.provider": "プロバイダー",
    "settings.providerCustom": "カスタム",
    "settings.cloudflareHint":
      "Cloudflare Workers AI も OpenAI 形式で話せます：base URL の ACCOUNT_ID を自分のアカウント ID に置き換え、モデルは {model} のようなものを使ってください。",
    "settings.groqHint":
      "Groq なら Whisper を無料で使えます。おすすめモデルは whisper-large-v3-turbo です。",
    "settings.getApiKey":
      "{host} で API キーを作成して下に貼り付け、「{action}」をクリックしてモデルを選んでください。",
    "settings.getApiKeyLink": "API キーを作成 →",
    "settings.ollamaHint": "Ollama は自分のマシン上で動くので、API キーは不要です。",
    "settings.baseUrl": "Base URL",
    "settings.apiKey": "API キー",
    "settings.model": "モデル",
    "settings.show": "表示",
    "settings.hide": "隠す",
    "settings.fetchModels": "モデル一覧を取得",
    "settings.fetching": "取得中…",
    "settings.test": "接続テスト",
    "settings.testing": "テスト中…",
    "settings.answered": "✓ 応答あり。",
    "settings.modelsFound": "{count} 件のモデルが見つかりました。",
    "settings.discard": "保存していない変更を破棄しますか？",
    "settings.exportSettings": "エクスポート",
    "settings.importSettings": "インポート",
    "settings.exportTitle": "設定のエクスポート",
    "settings.exportWarning":
      "下記は設定の文字列です。他の端末に貼り付ければ同じ設定をすぐに取り込めます。",
    "settings.copy": "コピー",
    "settings.copied": "コピーしました",
    "settings.copyFailed": "コピーできませんでした。テキストを選んで自分でコピーしてください。",
    "settings.importTitle": "クイックインポート",
    "settings.cipherPlaceholder": "設定の文字列を入力",
    "settings.importApply": "インポートを確定",
    "settings.importFailed": "インポートに失敗しました。文字列を解析できません。",
    "settings.imported": "入力しました。保存を押すと反映されます。",
  },

  ko: {
    "ask.prompt": "이 문장을 이해하도록 도와주세요: {text}",

    "theme.auto": "자동",
    "theme.light": "밝게",
    "theme.dark": "어둡게",
    "nav.settings": "설정",
    "nav.library": "라이브러리",
    "nav.search": "검색 또는 가져오기",

    "common.loading": "불러오는 중…",
    "common.back": "뒤로",
    "common.save": "저장",
    "common.saving": "저장 중…",
    "common.saved": "저장됨",
    "common.delete": "삭제",
    "common.confirm": "확인",
    "common.cancel": "취소",
    "common.close": "닫기",
    "common.on": "켜짐",
    "common.off": "꺼짐",

    "phase.queued": "대기 중",
    "phase.fetching": "내려받는 중",
    "phase.transcribing": "전사 중",
    "phase.annotating": "번역 중",
    "phase.ready": "완료",
    "phase.failed": "실패",
    "phase.untranscribed": "오디오만",

    "onboard.title": "세 단계로 시작하기",
    "onboard.proxy": "설정에서 바이트 프록시를 입력하세요. 모든 다운로드가 이것을 거칩니다.",
    "onboard.transcribe":
      "선택: 전사 모델을 등록하면 가져올 때 대본이 만들어집니다. 없어도 먼저 들을 수 있습니다.",
    "onboard.pick": "아래에서 방송을 고른 다음 가져올 에피소드를 선택하세요.",
    "onboard.done": "완료",

    "library.needsSettings": "먼저 설정에서 바이트 프록시를 구성하세요.",
    "library.continue": "이어 듣기",
    "library.left": "{time} 남음",
    "library.finished": "다 들음",
    "library.exportBackup": "내보내기",
    "library.importBackup": "백업 가져오기",
    "library.imported": "{added}개 추가, {skipped}개는 이미 있습니다.",
    "library.retry": "다시 시도",
    "library.resumeImport": "이어서",
    "library.working": "처리 중…",
    "library.autoLanguage": "자동",
    "library.pickEpisode": "가져올 에피소드를 선택하세요",
    "library.favorite": "즐겨찾기",
    "library.unfavorite": "즐겨찾기 해제",
    "library.columnTitle": "에피소드 제목",
    "library.columnDuration": "길이",
    "library.fullTitle": "전체 제목 보기",
    "library.feedFailed": "에피소드 목록을 가져오지 못했습니다: {reason}",

    "discover.title": "추천 팟캐스트",
    "discover.subtitle":
      "학습 중인 언어의 방송입니다. Apple Podcasts에서 가져와 하루에 한 번 갱신합니다.",
    "discover.loading": "팟캐스트를 찾는 중…",
    "discover.failed": "팟캐스트 디렉터리에 연결하지 못했습니다.",
    "discover.empty": "그 언어로는 결과가 없습니다.",
    "discover.favorites": "즐겨찾기",
    "discover.favoritesSubtitle": "내가 즐겨찾기한 팟캐스트",
    "discover.noFavorites": "즐겨찾기한 프로그램이 아직 없습니다",
    "discover.searchPlaceholder": "팟캐스트 검색 또는 RSS 링크 붙여넣기",
    "discover.results": "“{term}” 검색 결과",
    "discover.clearSearch": "검색 지우기",
    "discover.languageLearning": "언어 학습",

    "player.translating": "번역 중…",
    "player.missing": "해당 에피소드가 이 라이브러리에 없습니다.",
    "player.noTranscript": "아직 전사본이 없습니다.",
    "player.needsTranscription":
      "대본이 없습니다. 설정에서 전사 모델을 등록한 다음 다시 시도하세요.",
    "player.retryTranscription": "전사 다시 시도",
    "player.play": "재생",
    "player.playing": "재생 중",
    "player.paused": "일시정지됨",
    "player.position": "재생 위치",
    "player.pause": "일시정지",
    "player.previousLine": "이전 줄",
    "player.nextLine": "다음 줄",
    "player.lineOf": "{total}줄 중 {n}번째",
    "player.jumpToCurrent": "현재 줄로 이동",
    "player.replay": "현재 줄 다시 듣기",
    "player.replayShort": "다시",
    "player.repeat": "반복",
    "player.repeatTitle": "현재 줄 반복",
    "player.speed": "재생 속도",
    "player.subtitles": "자막",
    "player.showBoth": "함께",
    "player.showText": "원문",
    "player.showNone": "숨김",
    "player.revealHint": "자막이 숨겨져 있습니다. 줄을 고르면 다시 재생하며 보여 줍니다.",
    "player.askTitle": "이 문장에 대해 묻기",
    "player.asking": "질문 중…",
    "player.needsTextModel": "줄에 대해 질문하려면 설정에서 텍스트 모델을 등록하세요.",
    "player.keyPlay": "재생",
    "player.keyLine": "줄 이동",
    "player.keyReplay": "다시",
    "player.keyLoop": "반복",
    "player.keySubtitles": "자막",

    "settings.appearance": "모양",
    "settings.appearanceHint": "이 기기에만 적용되며 바로 반영됩니다.",
    "settings.look": "스타일",
    "settings.colorMode": "화면 모드",
    "look.standard": "기본",
    "look.standardHint": "음악 앱의 가사 화면처럼",
    "look.station": "역",
    "look.stationHint": "역 안내 전광판처럼",

    "settings.languages": "언어",
    "settings.proxy": "바이트 프록시",
    "settings.proxyKey": "프록시 키",
    "settings.proxyHint": "오디오를 가져오는 Worker입니다. 가져오기에 필요합니다.",

    "settings.native": "모국어",
    "settings.studying": "학습 언어 (선택)",
    "settings.autoDetect": "자동 감지",
    "settings.textModel": "텍스트 모델 — 번역과 AI 질문",
    "settings.transcriptionModel": "전사 모델 — 음성 인식",
    "settings.provider": "제공업체",
    "settings.providerCustom": "사용자 지정",
    "settings.cloudflareHint":
      "Cloudflare Workers AI도 OpenAI 형식을 지원합니다: base URL의 ACCOUNT_ID를 본인 계정 ID로 바꾸고, 모델은 {model} 같은 것을 사용하세요.",
    "settings.groqHint":
      "Groq는 Whisper를 무료로 제공합니다. 추천 모델은 whisper-large-v3-turbo입니다.",
    "settings.getApiKey":
      "{host}에서 API 키를 만들어 아래에 붙여넣은 다음 “{action}”을 눌러 모델을 선택하세요.",
    "settings.getApiKeyLink": "API 키 만들기 →",
    "settings.ollamaHint": "Ollama는 내 컴퓨터에서 실행되므로 API 키가 필요 없습니다.",
    "settings.baseUrl": "Base URL",
    "settings.apiKey": "API 키",
    "settings.model": "모델",
    "settings.show": "보기",
    "settings.hide": "숨기기",
    "settings.fetchModels": "모델 목록 가져오기",
    "settings.fetching": "가져오는 중…",
    "settings.test": "연결 테스트",
    "settings.testing": "테스트 중…",
    "settings.answered": "✓ 응답함.",
    "settings.modelsFound": "모델 {count}개를 찾았습니다.",
    "settings.discard": "저장하지 않은 변경을 버릴까요?",
    "settings.exportSettings": "내보내기",
    "settings.importSettings": "가져오기",
    "settings.exportTitle": "설정 내보내기",
    "settings.exportWarning":
      "아래는 설정 문자열입니다. 다른 기기에 붙여넣으면 동일한 설정을 빠르게 가져올 수 있습니다.",
    "settings.copy": "복사",
    "settings.copied": "복사됨",
    "settings.copyFailed": "복사하지 못했습니다. 텍스트를 선택해 직접 복사하세요.",
    "settings.importTitle": "빠른 가져오기",
    "settings.cipherPlaceholder": "설정 문자열 입력",
    "settings.importApply": "가져오기 확인",
    "settings.importFailed": "가져오기 실패: 문자열을 해석할 수 없습니다.",
    "settings.imported": "채웠습니다. 저장을 눌러야 유지됩니다.",
  },

  es: {
    "ask.prompt": "Ayúdame a entender esta frase: {text}",

    "theme.auto": "Auto",
    "theme.light": "Claro",
    "theme.dark": "Oscuro",
    "nav.settings": "Ajustes",
    "nav.library": "Biblioteca",
    "nav.search": "Buscar o importar",

    "common.loading": "Cargando…",
    "common.back": "Volver",
    "common.save": "Guardar",
    "common.saving": "Guardando…",
    "common.saved": "Guardado",
    "common.delete": "Eliminar",
    "common.confirm": "Confirmar",
    "common.cancel": "Cancelar",
    "common.close": "Cerrar",
    "common.on": "activado",
    "common.off": "desactivado",

    "phase.queued": "En cola",
    "phase.fetching": "Descargando",
    "phase.transcribing": "Transcribiendo",
    "phase.annotating": "Traduciendo",
    "phase.ready": "Listo",
    "phase.failed": "Falló",
    "phase.untranscribed": "Solo audio",

    "onboard.title": "Empieza en tres pasos",
    "onboard.proxy":
      "Configura el proxy de bytes en Ajustes: todas las descargas pasan por él.",
    "onboard.transcribe":
      "Opcional: añade un modelo de transcripción y cada importación llegará con su transcripción. Sin él ya puedes escuchar.",
    "onboard.pick": "Elige un programa abajo y luego un episodio para importar.",
    "onboard.done": "hecho",

    "library.needsSettings": "Configura primero el proxy de bytes en Ajustes.",
    "library.continue": "Seguir escuchando",
    "library.left": "Quedan {time}",
    "library.finished": "Terminado",
    "library.exportBackup": "Exportar",
    "library.importBackup": "Restaurar",
    "library.imported": "{added} añadidos, {skipped} ya estaban.",
    "library.retry": "Reintentar",
    "library.resumeImport": "Reanudar",
    "library.working": "Trabajando…",
    "library.autoLanguage": "auto",
    "library.pickEpisode": "Elige un episodio para importar",
    "library.favorite": "Favorito",
    "library.unfavorite": "Quitar favorito",
    "library.columnTitle": "Título del episodio",
    "library.columnDuration": "Duración",
    "library.fullTitle": "Ver el título completo",
    "library.feedFailed": "No se pudo obtener la lista de episodios: {reason}",

    "discover.title": "Pódcasts recomendados",
    "discover.subtitle":
      "Programas en el idioma que estudias, de Apple Podcasts. Se actualiza una vez al día.",
    "discover.loading": "Buscando pódcasts…",
    "discover.failed": "No se pudo acceder al directorio de pódcasts.",
    "discover.empty": "No hay nada para ese idioma.",
    "discover.favorites": "Favoritos",
    "discover.favoritesSubtitle": "Mis pódcasts favoritos",
    "discover.noFavorites": "Aún no hay programas favoritos",
    "discover.searchPlaceholder": "Busca pódcasts o pega un enlace RSS",
    "discover.results": "Resultados de «{term}»",
    "discover.clearSearch": "Borrar búsqueda",
    "discover.languageLearning": "Aprendizaje de idiomas",

    "player.translating": "Traduciendo…",
    "player.missing": "Ese episodio ya no está en esta biblioteca.",
    "player.noTranscript": "Aún no hay transcripción.",
    "player.needsTranscription":
      "Aún no hay transcripción. Configura un modelo de transcripción en Ajustes y vuelve a intentarlo.",
    "player.retryTranscription": "Reintentar la transcripción",
    "player.play": "Reproducir",
    "player.playing": "Reproduciendo",
    "player.paused": "En pausa",
    "player.position": "Posición",
    "player.pause": "Pausar",
    "player.previousLine": "Línea anterior",
    "player.nextLine": "Línea siguiente",
    "player.lineOf": "Línea {n} de {total}",
    "player.jumpToCurrent": "Ir a la línea actual",
    "player.replay": "Volver a oír la línea",
    "player.replayShort": "Otra vez",
    "player.repeat": "Repetir",
    "player.repeatTitle": "Repetir la línea actual",
    "player.speed": "Velocidad de reproducción",
    "player.subtitles": "Subtítulos",
    "player.showBoth": "Ambos",
    "player.showText": "Original",
    "player.showNone": "Ocultos",
    "player.revealHint": "Las líneas están ocultas. Elige una para volver a oírla y verla.",
    "player.askTitle": "Preguntar sobre esta línea",
    "player.asking": "Preguntando…",
    "player.needsTextModel":
      "Configura un modelo de texto en Ajustes para preguntar por una línea.",
    "player.keyPlay": "reproducir",
    "player.keyLine": "línea",
    "player.keyReplay": "otra vez",
    "player.keyLoop": "repetir",
    "player.keySubtitles": "subtítulos",

    "settings.appearance": "Apariencia",
    "settings.appearanceHint": "Solo en este dispositivo, y se aplica al instante.",
    "settings.look": "Estilo",
    "settings.colorMode": "Claro u oscuro",
    "look.standard": "Estándar",
    "look.standardHint": "Como la letra en una app de música",
    "look.station": "Estación",
    "look.stationHint": "Como el panel de salidas de una estación",

    "settings.languages": "Idiomas",
    "settings.proxy": "Proxy de bytes",
    "settings.proxyKey": "Clave del proxy",
    "settings.proxyHint": "El Worker que descarga el audio. Las importaciones lo necesitan.",

    "settings.native": "Idioma nativo",
    "settings.studying": "Estudiando (opcional)",
    "settings.autoDetect": "Detección automática",
    "settings.textModel": "Modelo de texto — traducción y preguntas a la IA",
    "settings.transcriptionModel": "Modelo de transcripción — voz a texto",
    "settings.provider": "Proveedor",
    "settings.providerCustom": "Personalizado",
    "settings.cloudflareHint":
      "Cloudflare Workers AI también habla el formato de OpenAI: cambia ACCOUNT_ID en la URL base por el tuyo, y usa un modelo como {model}.",
    "settings.groqHint":
      "Groq ofrece Whisper gratis — prueba el modelo whisper-large-v3-turbo.",
    "settings.getApiKey":
      "Crea una clave de API en {host}, pégala abajo y luego pulsa «{action}» para elegir un modelo.",
    "settings.getApiKeyLink": "Crear una clave de API →",
    "settings.ollamaHint": "Ollama funciona en tu propio equipo — no hace falta clave de API.",
    "settings.baseUrl": "URL base",
    "settings.apiKey": "Clave de API",
    "settings.model": "Modelo",
    "settings.show": "Mostrar",
    "settings.hide": "Ocultar",
    "settings.fetchModels": "Obtener modelos",
    "settings.fetching": "Obteniendo…",
    "settings.test": "Probar conexión",
    "settings.testing": "Probando…",
    "settings.answered": "✓ Respondió.",
    "settings.modelsFound": "{count} modelos encontrados.",
    "settings.discard": "¿Descartar los cambios sin guardar?",
    "settings.exportSettings": "Exportar",
    "settings.importSettings": "Importar",
    "settings.exportTitle": "Exportar configuración",
    "settings.exportWarning":
      "Esta es tu cadena de configuración. Cópiala y pégala en otro dispositivo para importar la misma configuración al instante.",
    "settings.copy": "Copiar",
    "settings.copied": "Copiado",
    "settings.copyFailed": "No se pudo copiar: selecciona el texto y cópialo tú.",
    "settings.importTitle": "Importación rápida",
    "settings.cipherPlaceholder": "Introduce la cadena de configuración",
    "settings.importApply": "Confirmar importación",
    "settings.importFailed": "Error al importar: no se pudo interpretar la cadena.",
    "settings.imported": "Rellenados: pulsa Guardar para conservarlos.",
  },

  fr: {
    "ask.prompt": "Aide-moi à comprendre cette phrase : {text}",

    "theme.auto": "Auto",
    "theme.light": "Clair",
    "theme.dark": "Sombre",
    "nav.settings": "Réglages",
    "nav.library": "Bibliothèque",
    "nav.search": "Rechercher ou importer",

    "common.loading": "Chargement…",
    "common.back": "Retour",
    "common.save": "Enregistrer",
    "common.saving": "Enregistrement…",
    "common.saved": "Enregistré",
    "common.delete": "Supprimer",
    "common.confirm": "Confirmer",
    "common.cancel": "Annuler",
    "common.close": "Fermer",
    "common.on": "activé",
    "common.off": "désactivé",

    "phase.queued": "En attente",
    "phase.fetching": "Téléchargement",
    "phase.transcribing": "Transcription",
    "phase.annotating": "Traduction",
    "phase.ready": "Prêt",
    "phase.failed": "Échec",
    "phase.untranscribed": "Audio seul",

    "onboard.title": "Commencer en trois étapes",
    "onboard.proxy":
      "Renseignez le proxy d’octets dans les Réglages : tous les téléchargements passent par lui.",
    "onboard.transcribe":
      "Facultatif : ajoutez un modèle de transcription pour que chaque import arrive transcrit. Sans lui, vous pouvez déjà écouter.",
    "onboard.pick": "Choisissez une émission ci-dessous, puis un épisode à importer.",
    "onboard.done": "fait",

    "library.needsSettings": "Configurez d'abord le proxy d'octets dans les Réglages.",
    "library.continue": "Reprendre l’écoute",
    "library.left": "Encore {time}",
    "library.finished": "Terminé",
    "library.exportBackup": "Exporter",
    "library.importBackup": "Restaurer",
    "library.imported": "{added} ajoutés, {skipped} déjà présents.",
    "library.retry": "Réessayer",
    "library.resumeImport": "Reprendre",
    "library.working": "En cours…",
    "library.autoLanguage": "auto",
    "library.pickEpisode": "Choisissez un épisode à importer",
    "library.favorite": "Favori",
    "library.unfavorite": "Retirer des favoris",
    "library.columnTitle": "Titre de l’épisode",
    "library.columnDuration": "Durée",
    "library.fullTitle": "Voir le titre complet",
    "library.feedFailed": "Impossible de récupérer la liste des épisodes : {reason}",

    "discover.title": "Podcasts recommandés",
    "discover.subtitle":
      "Des émissions dans la langue que vous étudiez, depuis Apple Podcasts. Mise à jour une fois par jour.",
    "discover.loading": "Recherche de podcasts…",
    "discover.failed": "Impossible de joindre l'annuaire des podcasts.",
    "discover.empty": "Rien pour cette langue.",
    "discover.favorites": "Favoris",
    "discover.favoritesSubtitle": "Mes podcasts favoris",
    "discover.noFavorites": "Aucune émission en favori pour l'instant",
    "discover.searchPlaceholder": "Cherchez un podcast ou collez un lien RSS",
    "discover.results": "Résultats pour « {term} »",
    "discover.clearSearch": "Effacer la recherche",
    "discover.languageLearning": "Apprentissage des langues",

    "player.translating": "Traduction…",
    "player.missing": "Cet épisode n'est plus dans cette bibliothèque.",
    "player.noTranscript": "Pas encore de transcription.",
    "player.needsTranscription":
      "Pas encore de transcription. Configurez un modèle de transcription dans les Réglages, puis réessayez.",
    "player.retryTranscription": "Relancer la transcription",
    "player.play": "Lecture",
    "player.playing": "Lecture",
    "player.paused": "En pause",
    "player.position": "Position",
    "player.pause": "Pause",
    "player.previousLine": "Ligne précédente",
    "player.nextLine": "Ligne suivante",
    "player.lineOf": "Ligne {n} sur {total}",
    "player.jumpToCurrent": "Aller à la ligne en cours",
    "player.replay": "Rejouer la ligne",
    "player.replayShort": "Rejouer",
    "player.repeat": "Répéter",
    "player.repeatTitle": "Répéter la ligne en cours",
    "player.speed": "Vitesse de lecture",
    "player.subtitles": "Sous-titres",
    "player.showBoth": "Les deux",
    "player.showText": "Original",
    "player.showNone": "Masqués",
    "player.revealHint":
      "Les lignes sont masquées. Choisissez-en une pour la réécouter et l’afficher.",
    "player.askTitle": "Poser une question sur cette ligne",
    "player.asking": "Question en cours…",
    "player.needsTextModel":
      "Configurez un modèle de texte dans les Réglages pour poser une question sur une ligne.",
    "player.keyPlay": "lecture",
    "player.keyLine": "ligne",
    "player.keyReplay": "rejouer",
    "player.keyLoop": "boucle",
    "player.keySubtitles": "sous-titres",

    "settings.appearance": "Apparence",
    "settings.appearanceHint": "Sur cet appareil seulement, et appliqué tout de suite.",
    "settings.look": "Style",
    "settings.colorMode": "Clair ou sombre",
    "look.standard": "Standard",
    "look.standardHint": "Comme les paroles d’une appli de musique",
    "look.station": "Gare",
    "look.stationHint": "Comme le tableau des départs d’une gare",

    "settings.languages": "Langues",
    "settings.proxy": "Proxy d'octets",
    "settings.proxyKey": "Clé du proxy",
    "settings.proxyHint": "Le Worker qui récupère l'audio. Les imports en ont besoin.",

    "settings.native": "Langue maternelle",
    "settings.studying": "Langue étudiée (facultatif)",
    "settings.autoDetect": "Détection automatique",
    "settings.textModel": "Modèle de texte — traduction et questions à l'IA",
    "settings.transcriptionModel": "Modèle de transcription — reconnaissance vocale",
    "settings.provider": "Fournisseur",
    "settings.providerCustom": "Personnalisé",
    "settings.cloudflareHint":
      "Cloudflare Workers AI parle aussi le format OpenAI : remplacez ACCOUNT_ID dans l'URL de base par le vôtre, et utilisez un modèle comme {model}.",
    "settings.groqHint":
      "Groq propose Whisper gratuitement — essayez le modèle whisper-large-v3-turbo.",
    "settings.getApiKey":
      "Créez une clé API sur {host}, collez-la ci-dessous, puis cliquez sur « {action} » pour en choisir un.",
    "settings.getApiKeyLink": "Créer une clé API →",
    "settings.ollamaHint": "Ollama tourne sur votre propre machine — aucune clé API requise.",
    "settings.baseUrl": "URL de base",
    "settings.apiKey": "Clé d'API",
    "settings.model": "Modèle",
    "settings.show": "Afficher",
    "settings.hide": "Masquer",
    "settings.fetchModels": "Lister les modèles",
    "settings.fetching": "Chargement…",
    "settings.test": "Tester la connexion",
    "settings.testing": "Test en cours…",
    "settings.answered": "✓ A répondu.",
    "settings.modelsFound": "{count} modèles trouvés.",
    "settings.discard": "Abandonner les modifications non enregistrées ?",
    "settings.exportSettings": "Exporter",
    "settings.importSettings": "Importer",
    "settings.exportTitle": "Exporter la configuration",
    "settings.exportWarning":
      "Voici votre chaîne de configuration. Copiez-la et collez-la sur un autre appareil pour importer rapidement la même configuration.",
    "settings.copy": "Copier",
    "settings.copied": "Copié",
    "settings.copyFailed": "Copie impossible — sélectionnez le texte et copiez-le vous-même.",
    "settings.importTitle": "Importation rapide",
    "settings.cipherPlaceholder": "Saisissez la chaîne de configuration",
    "settings.importApply": "Confirmer l'importation",
    "settings.importFailed": "Échec de l'importation : impossible d'analyser la chaîne.",
    "settings.imported": "Remplis — appuyez sur Enregistrer pour les garder.",
  },

  de: {
    "ask.prompt": "Hilf mir, diesen Satz zu verstehen: {text}",

    "theme.auto": "Auto",
    "theme.light": "Hell",
    "theme.dark": "Dunkel",
    "nav.settings": "Einstellungen",
    "nav.library": "Bibliothek",
    "nav.search": "Suchen oder importieren",

    "common.loading": "Lädt…",
    "common.back": "Zurück",
    "common.save": "Speichern",
    "common.saving": "Speichert…",
    "common.saved": "Gespeichert",
    "common.delete": "Löschen",
    "common.confirm": "Bestätigen",
    "common.cancel": "Abbrechen",
    "common.close": "Schließen",
    "common.on": "an",
    "common.off": "aus",

    "phase.queued": "In der Warteschlange",
    "phase.fetching": "Lädt herunter",
    "phase.transcribing": "Transkribiert",
    "phase.annotating": "Übersetzt",
    "phase.ready": "Fertig",
    "phase.failed": "Fehlgeschlagen",
    "phase.untranscribed": "Nur Audio",

    "onboard.title": "In drei Schritten loslegen",
    "onboard.proxy":
      "Richte in den Einstellungen den Byte-Proxy ein – jeder Download läuft darüber.",
    "onboard.transcribe":
      "Optional: Mit einem Transkriptionsmodell kommt jeder Import mit Transkript. Ohne kannst du trotzdem schon hören.",
    "onboard.pick": "Wähle unten eine Sendung und dann eine Folge zum Importieren.",
    "onboard.done": "erledigt",

    "library.needsSettings": "Richte zuerst den Byte-Proxy in den Einstellungen ein.",
    "library.continue": "Weiterhören",
    "library.left": "Noch {time}",
    "library.finished": "Fertig gehört",
    "library.exportBackup": "Exportieren",
    "library.importBackup": "Wiederherstellen",
    "library.imported": "{added} hinzugefügt, {skipped} schon vorhanden.",
    "library.retry": "Erneut versuchen",
    "library.resumeImport": "Fortsetzen",
    "library.working": "Arbeitet…",
    "library.autoLanguage": "auto",
    "library.pickEpisode": "Wähle eine Folge zum Importieren",
    "library.favorite": "Favorit",
    "library.unfavorite": "Favorit entfernen",
    "library.columnTitle": "Folgentitel",
    "library.columnDuration": "Dauer",
    "library.fullTitle": "Vollständigen Titel anzeigen",
    "library.feedFailed": "Die Episodenliste konnte nicht geladen werden: {reason}",

    "discover.title": "Empfohlene Podcasts",
    "discover.subtitle":
      "Sendungen in der Sprache, die du lernst, von Apple Podcasts. Einmal täglich aktualisiert.",
    "discover.loading": "Sucht Podcasts…",
    "discover.failed": "Das Podcast-Verzeichnis ist nicht erreichbar.",
    "discover.empty": "Für diese Sprache kam nichts zurück.",
    "discover.favorites": "Favoriten",
    "discover.favoritesSubtitle": "Meine Lieblingspodcasts",
    "discover.noFavorites": "Noch keine Lieblingssendungen",
    "discover.searchPlaceholder": "Podcasts suchen oder RSS-Link einfügen",
    "discover.results": "Ergebnisse für „{term}“",
    "discover.clearSearch": "Suche löschen",
    "discover.languageLearning": "Sprachen lernen",

    "player.translating": "Übersetzt…",
    "player.missing": "Diese Folge ist nicht mehr in dieser Bibliothek.",
    "player.noTranscript": "Noch kein Transkript.",
    "player.needsTranscription":
      "Noch kein Transkript. Richte in den Einstellungen ein Transkriptionsmodell ein und versuche es erneut.",
    "player.retryTranscription": "Transkription wiederholen",
    "player.play": "Abspielen",
    "player.playing": "Wiedergabe",
    "player.paused": "Pausiert",
    "player.position": "Position",
    "player.pause": "Pause",
    "player.previousLine": "Vorherige Zeile",
    "player.nextLine": "Nächste Zeile",
    "player.lineOf": "Zeile {n} von {total}",
    "player.jumpToCurrent": "Zur aktuellen Zeile",
    "player.replay": "Zeile erneut abspielen",
    "player.replayShort": "Nochmal",
    "player.repeat": "Wiederholen",
    "player.repeatTitle": "Aktuelle Zeile wiederholen",
    "player.speed": "Wiedergabegeschwindigkeit",
    "player.subtitles": "Untertitel",
    "player.showBoth": "Beides",
    "player.showText": "Original",
    "player.showNone": "Verborgen",
    "player.revealHint":
      "Die Zeilen sind verborgen. Wähle eine, um sie erneut zu hören und zu sehen.",
    "player.askTitle": "Zu dieser Zeile fragen",
    "player.asking": "Fragt…",
    "player.needsTextModel":
      "Richte in den Einstellungen ein Textmodell ein, um zu einer Zeile zu fragen.",
    "player.keyPlay": "Abspielen",
    "player.keyLine": "Zeile",
    "player.keyReplay": "Nochmal",
    "player.keyLoop": "Schleife",
    "player.keySubtitles": "Untertitel",

    "settings.appearance": "Erscheinungsbild",
    "settings.appearanceHint": "Nur auf diesem Gerät, und sofort wirksam.",
    "settings.look": "Stil",
    "settings.colorMode": "Hell oder dunkel",
    "look.standard": "Standard",
    "look.standardHint": "Wie die Songtext-Ansicht einer Musik-App",
    "look.station": "Bahnhof",
    "look.stationHint": "Wie die Abfahrtstafel am Bahnhof",

    "settings.languages": "Sprachen",
    "settings.proxy": "Byte-Proxy",
    "settings.proxyKey": "Proxy-Schlüssel",
    "settings.proxyHint": "Der Worker, der das Audio abruft. Importe brauchen ihn.",

    "settings.native": "Muttersprache",
    "settings.studying": "Lernsprache (optional)",
    "settings.autoDetect": "Automatisch erkennen",
    "settings.textModel": "Textmodell — Übersetzung und KI-Fragen",
    "settings.transcriptionModel": "Transkriptionsmodell — Sprache zu Text",
    "settings.provider": "Anbieter",
    "settings.providerCustom": "Benutzerdefiniert",
    "settings.cloudflareHint":
      "Cloudflare Workers AI spricht ebenfalls das OpenAI-Format: ersetze ACCOUNT_ID in der Basis-URL durch deine eigene, und nutze ein Modell wie {model}.",
    "settings.groqHint":
      "Groq bietet Whisper kostenlos — probier das Modell whisper-large-v3-turbo.",
    "settings.getApiKey":
      "Erstelle einen API-Schlüssel bei {host}, füge ihn unten ein und klicke dann auf „{action}“, um eins auszuwählen.",
    "settings.getApiKeyLink": "API-Schlüssel erstellen →",
    "settings.ollamaHint":
      "Ollama läuft auf deinem eigenen Rechner — kein API-Schlüssel nötig.",
    "settings.baseUrl": "Basis-URL",
    "settings.apiKey": "API-Schlüssel",
    "settings.model": "Modell",
    "settings.show": "Anzeigen",
    "settings.hide": "Ausblenden",
    "settings.fetchModels": "Modelle laden",
    "settings.fetching": "Lädt…",
    "settings.test": "Verbindung testen",
    "settings.testing": "Testet…",
    "settings.answered": "✓ Hat geantwortet.",
    "settings.modelsFound": "{count} Modelle gefunden.",
    "settings.discard": "Nicht gespeicherte Änderungen verwerfen?",
    "settings.exportSettings": "Exportieren",
    "settings.importSettings": "Importieren",
    "settings.exportTitle": "Konfiguration exportieren",
    "settings.exportWarning":
      "Das ist deine Konfigurationszeichenkette. Kopiere sie und füge sie auf einem anderen Gerät ein, um dieselbe Konfiguration schnell zu übernehmen.",
    "settings.copy": "Kopieren",
    "settings.copied": "Kopiert",
    "settings.copyFailed": "Kopieren ging nicht — markier den Text und kopier ihn selbst.",
    "settings.importTitle": "Schnellimport",
    "settings.cipherPlaceholder": "Konfigurationszeichenkette eingeben",
    "settings.importApply": "Import bestätigen",
    "settings.importFailed":
      "Import fehlgeschlagen: Die Zeichenkette konnte nicht gelesen werden.",
    "settings.imported": "Ausgefüllt — zum Behalten auf Speichern drücken.",
  },
};

/**
 * One string in one language, with `{name}` placeholders filled in. An unknown
 * language falls back to English rather than showing the key — a missing
 * translation should read oddly, never break the screen.
 */
export function t(locale: LanguageCode, key: StringKey, vars?: Vars): string {
  const text = (STRINGS[locale] ?? STRINGS.en)[key] ?? STRINGS.en[key];
  return vars
    ? text.replace(/\{(\w+)\}/g, (whole, name: string) => String(vars[name] ?? whole))
    : text;
}

/**
 * A language's name as the reader's own interface would write it — "日语" in a
 * Simplified Chinese interface, "japonais" in a French one.
 *
 * `Intl.DisplayNames` rather than eight more names in each of the eight tables above:
 * this is one of the few pieces of UI text every browser already ships, in every
 * language, and sixty-four hand-written entries would be sixty-four entries to keep
 * in step for nothing. `LANGUAGE_NAMES` cannot do this job — it is English on purpose,
 * because its other caller names the language to the Text Model in a prompt.
 *
 * The two Chinese codes are asked for by *script*, not by the region they carry:
 * `of("zh-CN")` answers "中文（中国）" and `of("zh-TW")` "中文（台湾）", which name
 * countries where this setting means Simplified against Traditional. `zh-Hans` and
 * `zh-Hant` answer "简体中文" and "繁体中文", which is both correct and what the
 * English table has always called them.
 */
export function languageName(code: LanguageCode, locale: LanguageCode): string {
  const subtag = SCRIPT_SUBTAGS[code] ?? code;
  const named = new Intl.DisplayNames([locale], { type: "language" }).of(subtag);
  // `of` hands back the subtag itself when it has no data for it, and a button
  // labelled "zh-Hant" is worse than one labelled in English.
  return !named || named === subtag ? LANGUAGE_NAMES[code] : named;
}

/** Only the two that would otherwise be named after a country. */
const SCRIPT_SUBTAGS: Partial<Record<LanguageCode, string>> = {
  "zh-CN": "zh-Hans",
  "zh-TW": "zh-Hant",
};
